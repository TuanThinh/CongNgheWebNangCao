		...
		try {
			...
			if(listThuoc.size() == 0) {
				...
				if(countRequest> 2) {
					...
					redisUtil.setExpire(Constant.CONSTANT_SANPHAM, 1, TimeUnit.HOURS);
		...
		try {
			...
			if(sp == null) {
				...
				if(countRequest > 2) {
					...
					redisUtil.setExpire(Constant.CONSTANT_SANPHAM+"_"+id, 1, TimeUnit.DAYS);
				}
			}
			return sp;
		} catch (Exception e) {
...
//		}
	...
	public Thuoc save(Thuoc thuoc) {
		...
		try {
			...
			Thuoc newThuoc = new ObjectMapper().readValue(strThuoc, Thuoc.class);
			redisUtil.putMap(Constant.CONSTANT_SANPHAM, Constant.CONSTANT_SANPHAM+"_"+newThuoc.getId(), newThuoc);
			return newThuoc;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
